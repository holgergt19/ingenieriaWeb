...
class ContratosController extends AppController
{
    ...
    public function reporte()
    {
        ...
        if ($this->request->is('get')) {
            ...
            $contratos = $this->Contratos->find()
                ->select(['Cliente.nombre', 'total_montos' => 'SUM(Contratos.monto)'])
                ->join(['Clientes' => ['table' => 'clientes', 'type' => 'INNER', 'conditions' => 'Contratos.idCliente = Clientes.idCliente']])
                ->where(function ($exp, $q) use ($fechaInicio, $fechaFin) {
                    return $exp->between('Contratos.fecha', $fechaInicio, $fechaFin);
                })
                ->group('Contratos.idCliente');

            $this->set(compact('fechaInicio', 'fechaFin', 'contratos'));
        }
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Contratos->save($contrato)) {
                ...
                return $this->redirect(['action' => 'reporte']);
            }
            $this->Flash->error(__(' Please, try again.'));
        }
        $this->set(compact('contrato'));
        $clientes = $this->Contratos->find('all');
